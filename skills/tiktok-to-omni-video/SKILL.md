...
```bash
python3 /Users/<user>/.codex/skills/tiktok-to-omni-video/scripts/analyze_local_audio.py "<local-video-path>" --out-dir "<per-clip-audio-dir>"
```

The script must use the configured on-device MLX Whisper model and must not upload audio. It resolves `mlx_whisper` from `LOCAL_WHISPER_BIN`, `PATH`, and `~/Library/Python/*/bin/mlx_whisper`; it resolves the model from `LOCAL_WHISPER_MODEL` or `~/.cache/watch/whisper-small-mlx`. Run `--check` before a batch and stop with a clear remediation message if the executable, model, `ffmpeg`, or `ffprobe` is unavailable.
...
```bash
export OMNI_API_KEY='...'
python3 /Users/<user>/.codex/skills/tiktok-to-omni-video/scripts/generate_omni.py jobs.json --limit 1
```

Use `--limit 1` before review. Only after the user says `continue`/`继续`, run the same manifest and job list with `--all-remaining`:

```bash
python3 /Users/<user>/.codex/skills/tiktok-to-omni-video/scripts/generate_omni.py jobs.json --all-remaining
```
